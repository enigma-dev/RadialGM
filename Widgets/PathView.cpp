...
QPoint cubic(QPoint start, QPoint handle1, QPoint handle2, QPoint end,
             double position) {
...
int PathView::Size() const {
  RepeatedProtoModelPtr pointsModel =
      pathModel->GetRepeatedSubModel(Path::kPointsFieldNumber);
  return pointsModel->rowCount();
}

bool PathView::Closed() const {
  return pathModel->data(Path::kClosedFieldNumber).toBool();
}
bool PathView::Smooth() const {
  return pathModel->data(Path::kSmoothFieldNumber).toBool();
}
...
QPoint PathView::EffectivePoint(int n, int size, bool closed) const {
  return Point(EffectiveIndex(n, size, closed));
}
...
void shittyCurve(QPoint start, QPoint handle, QPoint end,
                 double increment, QVector<QPoint> *points) {
...
// damned spline is
void reallyStupidCurve(QPoint previous, QPoint current, QPoint next,
                       double increment, QVector<QPoint> *points) {
...
void PathView::Paint(QPainter &painter) {
  ...
  // FIXME: this is supposed to be the selected point in the model, not the last
  // point in the list.
  if (!closed && !user_points.empty()) {
    painter.setBrush(QBrush(Qt::red));
    painter.drawEllipse(user_points.back(), 4, 4);
  }
